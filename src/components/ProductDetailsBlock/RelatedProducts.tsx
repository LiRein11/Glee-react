...
import { IProducts, IProductsLength } from '../products.interface';
...
const RelatedProducts: FC = () => {
  ...
  const { data: devices } = useQuery<IProducts & IProductsLength>(
    ['devices', null, null, pageNoSlice, limit],
    () => fetchDevices(null, null, pageNoSlice, limit),
  );
      ...
      <div className='container'>
        <div className='related__top'>
          ...
          <div className='related__box-buttons'>
            <Pagination
              devices={devices}
              limit={devices?.rows.length}
              count={devices ? devices.count : 0}
              pageNoSlice={pageNoSlice}
              setPageNoSlice={setPageNoSlice}
            />
          </div>
        </div>
        <div className='related__items related__items--active' id='tabs-1'>
          {devices?.rows.map((el, i) => (
            <ProductItem key={el.id} device={el} />
          ))}
        </div>
